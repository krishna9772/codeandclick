<x-app-layout>
    <x-slot name="header">
        <div class="flex w-full items-center">
            <a href="{{ route('our-work.index') }}" class="rounded border border-blue-800 px-4 py-2 font-bold text-blue-800">
        ...
        <div class="mx-auto max-w-5xl sm:px-6 lg:px-8">
            <div class="overflow-hidden rounded-lg bg-white shadow-sm">
                <div class="p-8">
                    ...
                    <form action="{{ route('our-work.update', $ourwork->id) }}" method="POST" enctype="multipart/form-data" id="ourWorkForm" class="space-y-8" novalidate>
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <div id="imageFieldWrapper" class="rounded-2xl border border-dashed border-gray-300 p-4">
                                <div id="imagePreviewWrapper" class="mb-4 overflow-hidden rounded-xl border border-gray-200">
                                    <img id="imagePreview" src="{{ $ourwork->getFirstMediaUrl('ourwork-header') }}" alt="Image preview" class="h-56 w-full object-cover">
                                </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title', $ourwork->title) }}" maxlength="255" class="w-full rounded-lg border border-gray-300 p-3" required>
                            @error('title')
                                <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <select name="serviceID" id="serviceID" class="w-full rounded-lg border border-gray-300 p-3" required>
                                ...
                                @foreach($services as $service)
                                    <option value="{{ $service->id }}" {{ (string) old('serviceID', $ourwork->serviceID) === (string) $service->id ? 'selected' : '' }}>
                                        {{ $service->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <select name="type" id="type" class="w-full rounded-lg border border-gray-300 p-3" required>
                                ...
                                <option value="branding-solution" {{ old('type', $ourwork->type) === 'branding-solution' ? 'selected' : '' }}>Branding Solution</option>
                                <option value="brand-strategy" {{ old('type', $ourwork->type) === 'brand-strategy' ? 'selected' : '' }}>Brand Strategy</option>
                                <option value="consultancy-integration-and-culture" {{ old('type', $ourwork->type) === 'consultancy-integration-and-culture' ? 'selected' : '' }}>Consultancy Integration and Culture</option>
                                <option value="brand-identity" {{ old('type', $ourwork->type) === 'brand-identity' ? 'selected' : '' }}>Brand Identity(Logo Design and Brand Book)</option>
                                <option value="marketing-services" {{ old('type', $ourwork->type) === 'marketing-services' ? 'selected' : '' }}>Marketing Services</option>
                                <option value="marketing-strategy" {{ old('type', $ourwork->type) === 'marketing-strategy' ? 'selected' : '' }}>Marketing Strategy and Consultancy Digital Marketing</option>
                                <option value="social-media" {{ old('type', $ourwork->type) === 'social-media' ? 'selected' : '' }}>Social Media</option>
                                <option value="search-engine-optimization" {{ old('type', $ourwork->type) === 'search-engine-optimization' ? 'selected' : '' }}>Search Engine Optimization(SEO)</option>
                                <option value="digital-optimization" {{ old('type', $ourwork->type) === 'digital-optimization' ? 'selected' : '' }}>Digital Optimization</option>
                                <option value="media-and-press" {{ old('type', $ourwork->type) === 'media-and-press' ? 'selected' : '' }}>Media and Press</option>
                                <option value="events-coverage-and-live-streaming" {{ old('type', $ourwork->type) === 'events-coverage-and-live-streaming' ? 'selected' : '' }}>Events Coverage and Live Streaming</option>
                                <option value="creative-design" {{ old('type', $ourwork->type) === 'creative-design' ? 'selected' : '' }}>Creative Design</option>
                                <option value="website-and-social-media-content" {{ old('type', $ourwork->type) === 'website-and-social-media-content' ? 'selected' : '' }}>Website and Social Media Content</option>
                                <option value="video-production" {{ old('type', $ourwork->type) === 'video-production' ? 'selected' : '' }}>Video Production</option>
                                <option value="motions" {{ old('type', $ourwork->type) === 'motions' ? 'selected' : '' }}>Motions</option>
                                <option value="photo-shooting" {{ old('type', $ourwork->type) === 'photo-shooting' ? 'selected' : '' }}>Photo Shooting</option>
                                <option value="mobile-app-development" {{ old('type', $ourwork->type) === 'mobile-app-development' ? 'selected' : '' }}>Mobile App Development</option>
                                <option value="web-design" {{ old('type', $ourwork->type) === 'web-design' ? 'selected' : '' }}>Web Design</option>
                            </select>
                            @error('type')
                                <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        ...
                        <div class="space-y-6 border-t border-gray-200 pt-8">
                            <div>
                                ...
                                <textarea name="content" id="content" class="w-full rounded-lg border border-gray-300 p-3" required data-required-message="The content field is required.">{{ old('content', $ourwork->content) }}</textarea>
                                <p id="content_error" class="mt-2 hidden text-sm text-red-500"></p>
                                @error('content')
                                    <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="space-y-6 border-t border-gray-200 pt-8">
                            <div>
                                ...
                                <div id="workImagesFieldWrapper" class="rounded-2xl border border-dashed border-gray-300 p-4">
                                    <div id="workImagesPreviewContainer" class="mb-4 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                                        @foreach ($ourwork->getMedia('ourwork-images') as $media)
                                            <div class="existing-work-image group relative" data-media-id="{{ $media->id }}">
                                                <img src="{{ $media->getUrl() }}" alt="Our Work Image" class="h-40 w-full rounded-lg border border-gray-200 object-cover">
                                                <button type="button" class="remove-existing-work-image absolute right-2 top-2 inline-flex h-7 w-7 items-center justify-center rounded-full bg-red-500 text-white opacity-0 transition-opacity group-hover:opacity-100" data-media-id="{{ $media->id }}">
                                    ...
                                    <input id="workImages" name="workImages[]" type="file" class="hidden" accept=".jpeg,.jpg,.png,.gif,image/jpeg,image/png,image/gif" multiple>
                        ...
                        <div class="space-y-6 border-t border-gray-200 pt-8">
                            ...
                            <div>
                                ...
                                <input type="text" name="title_mm" id="title_mm" value="{{ old('title_mm', $ourwork->title_mm) }}" maxlength="255" class="w-full rounded-lg border border-gray-300 p-3">
                                @error('title_mm')
                                    <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="content_mm" id="content_mm" class="w-full rounded-lg border border-gray-300 p-3">{{ old('content_mm', $ourwork->content_mm) }}</textarea>
                                @error('content_mm')
                                    <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function getExistingWorkImageCount() {
                return workImagesPreviewContainer.querySelectorAll('.existing-work-image').length;
            }

            function validateWorkImages() {
                setWorkImagesError('');

                if (!selectedWorkImages.length && !getExistingWorkImageCount()) {
                    setWorkImagesError('Please keep at least one work image.');
                    return false;
                }
            ...
            function updateWorkImagesCount() {
                const total = getExistingWorkImageCount() + selectedWorkImages.length;
                workImagesCount.textContent = total ? total + ' image(s) ready' : 'JPG, JPEG, PNG, or GIF up to 5MB each.';
            }

            function renderWorkImages() {
                workImagesPreviewContainer.querySelectorAll('.new-work-image').forEach(function(item) {
                    item.remove();
                });
                ...
                selectedWorkImages.forEach(function(file, index) {
                    ...
                    reader.onload = function(e) {
                        ...
                        previewDiv.className = 'new-work-image group relative';
