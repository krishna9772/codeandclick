    ...
    <div class="py-12">
        <div class="mx-auto max-w-5xl sm:px-6 lg:px-8">
            <div class="overflow-hidden rounded-lg bg-white shadow-sm">
                <div class="p-8">
                    <form action="{{ route('testimornials.update', $testimornial->id) }}" method="POST" enctype="multipart/form-data" id="testimornialForm" class="space-y-8" novalidate>
                        @csrf
                        @method('PUT')
                        ...
                        <div id="imageFieldWrapper">
                            ...
                            <div id="imageUploadBox" class="rounded-3xl border border-dashed border-gray-300 p-6 transition-colors">
                                <div id="imagePreviewContainer" class="mb-4 overflow-hidden rounded-2xl">
                                    <img id="imagePreview" src="{{ asset($testimornial->getFirstMediaUrl('testimornials')) }}" alt="Image Preview" class="h-64 w-full object-cover">
                                </div>
                                ...
                                <div class="flex flex-col gap-4">
                                    <div>
                                        ...
                                        <input id="image" name="image" type="file" class="sr-only" accept=".jpeg,.jpg,.png,.gif,image/jpeg,image/png,image/gif">
                                    </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="name" id="name" maxlength="255" class="w-full rounded-lg border border-gray-300 p-3" value="{{ old('name', $testimornial->name) }}" required>
                            @error('name')
                                <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <textarea rows="10" name="description" id="description" class="w-full rounded-lg border border-gray-300 p-3" required>{{ old('description', $testimornial->description) }}</textarea>
                            @error('description')
                                <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="space-y-6 border-t border-gray-200 pt-8">
                            ...
                            <div>
                                ...
                                <input type="text" name="name_mm" id="name_mm" maxlength="255" class="w-full rounded-lg border border-gray-300 p-3" value="{{ old('name_mm', $testimornial->name_mm) }}">
                                @error('name_mm')
                                    <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea rows="10" name="description_mm" id="description_mm" class="w-full rounded-lg border border-gray-300 p-3">{{ old('description_mm', $testimornial->description_mm) }}</textarea>
