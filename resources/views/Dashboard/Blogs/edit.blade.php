        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-8">
                    ...
                    <form action="{{ route('bloglist.update', $blog->id) }}" method="POST" enctype="multipart/form-data" id="blogEditForm" class="space-y-8">
                        @csrf
                        @method('PUT')
                        ...
                        <div>
                            ...
                            <div id="imageFieldWrapper" class="rounded-2xl border border-dashed border-gray-300 p-4">
                                <div id="imagePreviewWrapper" class="mb-4 overflow-hidden rounded-xl border border-gray-200">
                                    <img id="imagePreview" src="{{ $blog->getFirstMediaUrl('blog_images') ?: '#' }}" alt="Image preview" class="h-56 w-full object-cover">
                                </div>
                                ...
                                <p id="imageFileName" class="mt-3 text-sm text-gray-500">Leave empty to keep the current image.</p>
                        ...
                        <div>
                            ...
                            <input type="text" name="title" id="title" value="{{ old('title', $blog->title) }}" maxlength="255" class="w-full rounded-lg border border-gray-300 p-3" required>
                            @error('title')
                                <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                            @enderror
                        </div>
                                    ...
                                    @foreach ($blogTypes as $type)
                                        <label class="flex items-center gap-3 rounded-lg border border-gray-200 p-3">
                                            <input
                                                ...
                                                {{ in_array($type, old('type', $selectedTypes), true) ? 'checked' : '' }}>
                        ...
                        <div class="space-y-6 border-t border-gray-200 pt-8">
                            <div>
                                ...
                                <textarea name="content" id="content" class="w-full rounded-lg border border-gray-300 p-3" required>{{ old('content', $blog->content) }}</textarea>
                        ...
                        <div class="space-y-6 border-t border-gray-200 pt-8">
                            ...
                            <div>
                                ...
                                <input type="text" name="title_mm" id="title_mm" value="{{ old('title_mm', $blog->title_mm) }}" maxlength="255" class="w-full rounded-lg border border-gray-300 p-3">
                                @error('title_mm')
                                    <p class="mt-2 text-sm text-red-500">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="content_mm" id="content_mm" class="w-full rounded-lg border border-gray-300 p-3">{{ old('content_mm', $blog->content_mm) }}</textarea>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function validateImage() {
                ...
                if (!imageInput.files || !imageInput.files[0]) {
                    imageFileName.textContent = 'Leave empty to keep the current image.';
                    return true;
                }
